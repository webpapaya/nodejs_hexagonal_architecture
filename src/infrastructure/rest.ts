...
import {Dependencies, withDependencies} from "./dependencies";
...
export const endpoint = <T>(callback: (args: { req: Request, res: Response, dependencies: Dependencies }) => T) => {
  return (req: Request, res: Response) => withDependencies(async (dependencies) => {
    try {
      return await callback({req, res, dependencies})
    } catch (e) {
